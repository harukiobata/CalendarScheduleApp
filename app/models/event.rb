class Event < ApplicationRecord
  ...
  def no_time_overlap
    ...
    overlapping_bookings = Booking.where(owner_id: user_id).where("start_time < ? AND end_time > ?", end_time, start_time)

    if overlapping_events.exists? || overlapping_bookings.exists?
      conflict_titles = overlapping_events.pluck(:title).join(", ")
      errors.add(:title, "が重なっています#{conflict_titles.present? ? ": #{conflict_titles}" : ''}")
    end
  end
